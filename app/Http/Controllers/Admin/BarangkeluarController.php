...
class BarangkeluarController extends Controller
{
    ...
    $data['stokMenipis'] = BarangModel::where('barang_stok', '<=', 100)
        ->where('barang_stok', '>', 0)
        ->get();
    ...
    public function show(Request $request)
    {
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->rawColumns(['action', 'tgl','customer', 'tujuan', 'barang'])->make(true);
        }
    }
...
public function proses_ubah(
    Request $request,
    BarangkeluarModel $barangkeluar
)
{
    // ambil jumlah lama
    ...
    // hitung selisih
    $selisih = $request->jml - $jumlahLama;
...
)
{
    ...
    BarangModel::where(
        'barang_kode',
        $barangkeluar->barang_kode
    )->increment(
        'barang_stok',
        $barangkeluar->bk_jumlah
    );
